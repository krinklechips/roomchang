    ...
    <section id="services" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-2 font-display text-4xl text-[color:var(--text-main)] sm:text-5xl">
            Our Treatments
          </h2>
        </div>
        <Link
      ...
      <div className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        {SERVICES.map((service) => (
          <Link
            key={service.id}
            href={service.href}
            className="group flex flex-col items-center gap-3 rounded-2xl border border-[color:var(--border-strong)] bg-white px-4 py-6 text-center shadow-[0_8px_30px_rgba(57,28,45,0.05)] transition hover:-translate-y-0.5 hover:border-[color:var(--brand-light)] hover:shadow-[0_14px_40px_rgba(57,28,45,0.1)]"
          >
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[color:var(--surface-strong)] text-[color:var(--brand-deep)] transition group-hover:bg-[color:var(--brand-soft)]">
              <service.Icon size={28} strokeWidth={1.75} aria-hidden="true" />
            </div>
            <p className="text-[0.82rem] font-semibold leading-snug text-[color:var(--text-main)]">
              {service.label}
            </p>
          </Link>
        ))}
