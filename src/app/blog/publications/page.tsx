...
export const metadata: Metadata = {
  title: "Publications | Roomchang Dental Hospital",
  description:
    "Research papers and clinical studies published by Roomchang's specialist dentists.",
};

export default async function PublicationsPage() {
  const posts = await getBlogPosts("publications");
        ...
        {posts.length === 0 ? (
          <div className="rounded-3xl border border-[color:var(--border-strong)] bg-white p-12 text-center">
            ...
            <p className="mt-3 text-sm text-[color:var(--text-soft)]">
              Our team is preparing publications for this section. Check back soon.
            </p>
          ...
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {posts.map((post) => (
              <Link
                key={post.id}
                href={`/blog/publications/${post.slug}`}
