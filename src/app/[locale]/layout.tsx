...
// ─────────────────────────────────────────────────────────────────────────────
// This is the ROOT layout (there is intentionally NO src/app/layout.tsx).
// It lives inside [locale] so the locale comes from params — the previous root
// layout called getLocale(), a request-header API that forced EVERY route into
// dynamic rendering (zero prerendered pages, ~2s TTFB on each visit). With the
// locale from params + setRequestLocale, all ISR pages (revalidate = 60)
// actually prerender and serve from the CDN. next-intl documents this exact
// structure for static rendering. global-error.tsx renders its own <html>.
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * Namespaces needed by CLIENT components (files with "use client" that call
 * useTranslations). Server components use getTranslations and never need
 * messages in the browser. Passing getMessages() wholesale serialized the
 * ENTIRE ~1200-key catalog (~79KB) into every page's HTML — this pick list
 * keeps the flight payload to just what hydration actually reads.
 *
 * When adding useTranslations("x") to a "use client" component, add "x" here —
 * otherwise the browser renders the raw key path (loud, but broken).
 * NOTE: "chatbot" was removed when Roomy was hidden (site-shell.tsx) — re-add
 * it here if the <Chatbot /> integration is re-enabled.
 */
